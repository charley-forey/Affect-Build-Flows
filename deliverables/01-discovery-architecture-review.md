...
- [x] **Endpoint inventory — 44 endpoints**, generated from `config/endpoints.yml` so it cannot drift — [`endpoint-inventory.md`](../foundation/charley-dev/_docs/endpoint-inventory.md)
...
| 2026-08-02 | **Closed.** Endpoint inventory generated from the registry (44 endpoints, every path cited). Current-state assessment delivered as three documents rather than one: what is live (`build-status.md`), the independent audit (`assessment.md`), and the credential exposure found in the existing workspace (`security-findings.md`). |
| 2026-08-19 | Inventory regenerated after the registry grew; the count is **44**, not the 36 an earlier stale copy of the generated file reported. It went 42 → 44 the same day, when the PQP work added `checklist_lists` and `checklist_list_items`. The registry is the source of truth and the document is generated from it — which is exactly why the number moved twice in one day without anyone editing a doc. |
