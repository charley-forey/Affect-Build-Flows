...
| 2026-08-20 | **`fct_Milestone` repointed onto our own Outbuild ingestion** — 52 → 126 rows, 2 → 3 projects, 0 orphans. New silver parser `25_outbuild_silver.sql`; an Outbuild activity carries no project id, so the join runs `schedule_id → project.schedules[].id → project.procore_id`. Only **3 of 15** Outbuild projects carry a `procore_id`, so 280 of 406 critical activities are unattributable — a Procore-integration gap on Affect's side, and the reason the gain is 126 and not 406. |
| 2026-08-20 | **`dim_Job` built for the first time — 0 rows, and that is correct.** The chain (Job Register → bronze → silver → gold) ran end to end in the gold rebuild. The register is empty because no job has been requested yet and `CD_Manual_Ingest` is not signed in, so `dim_Job` reads as an honest blank rather than a broken join. |
