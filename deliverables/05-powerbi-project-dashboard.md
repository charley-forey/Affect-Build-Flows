...
- [ ] Answer the two remaining PQP trade questions with Affect — the 16-row `qc_seed_TradeAlias` recovered 464 records (NCRs 459 → **215**, punch 511 → **291**). What is left: three ambiguous labels deliberately not guessed (`Drywall/Carpentry` 255, `Concrete Superstructure` 110, `Concrete` 64), and a **scope** question — Roofing, Glazing, Windows, Structural Steel, Low Voltage and others have no equivalent trade in the 26-sheet checklist library at all. Both shown on the PQP Data Quality page rather than hidden
