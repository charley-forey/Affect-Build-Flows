...
**Status:** 🟡 Delivered inside the Monthly Progress Report — no standalone list built | **Phase:** 1 — Foundation | **Billing:** $125/hr — 3 hrs in Phase 0 | **Target:** Delivered, scope changed
...
>
> **The finding is more important than the deliverable.** All **105 of 105** certificates on file are expired, the most recent expiry is 2025-04-01, and only 23 of 251 vendors have a certificate at all — of those, 6 are on a current project. The likelier reading is that Procore's insurance module was populated once and abandoned, not that Affect's subcontractors are uninsured. Those are very different facts for a general contractor and nothing in the current reporting distinguishes them. The model therefore keeps **coverage** (is there a certificate?) separate from **currency** (is it in date?). **This is a question for Affect, not a conclusion from us.**
