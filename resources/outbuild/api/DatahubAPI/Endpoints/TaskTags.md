<!-- Source: https://pp-docs.outbuild.com/docs/DatahubAPI/Endpoints/TaskTags -->

# TaskTags

The endpoints described in this section retrieve information on task tags associated with active schedules in your organization.

# Task Tags

1. [Endpoint 1: GET all task tags](#request-tasktags)

## `GET` /tasktags

### Description

*This endpoint retrieves a list of and organization's task tags.*

> **📄 Pagination**: The endpoint returns **500** task tags per request  
...
|---|---|---|
| `https://datahub.outbuild.com/tasktags?page=1` | `authorizationToken` | `page` |
...
This endpoint accepts an optional `page` query parameter. For example, to request the second page of data, use the following URL: `https://datahub.outbuild.com/tasktags?page=2`.

If no `page` parameter is specified (e.g., `https://datahub.outbuild.com/tasktags`), the first page will be returned by default.

### Response

#### 🎉 Success

The endpoint returns a JSON object containing a list of task tags for a specific organization. If no task tags are available, the response body will be empty (`[]`).
...
**Click to view a detailed explanation of the fields for each taskTag**

| Name | Description | Type |
|---|---|---|
| `task_id` | Identifier of the task to which the tag is associated | Number |
...
Use the following curl example to connect to the production endpoint (substitute {taskId} with an integer value):

```bash
curl -X GET "https://datahub.outbuild.com/tasktags?page=1" \
