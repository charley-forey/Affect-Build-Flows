...
try {
    if (-not (Test-Field $list "UserPrincipalName")) { Add-PnPField -List $list -DisplayName "UserPrincipalName" -InternalName "UserPrincipalName" -Type Text -AddToDefaultView | Out-Null }
    if (-not (Test-Field $list "ProjectKey")) { Add-PnPField -List $list -DisplayName "ProjectKey" -InternalName "ProjectKey" -Type Lookup -AddToDefaultView -ErrorAction Stop | Out-Null
    Set-PnPField -List $list -Identity "ProjectKey" -Values @{ LookupList = $projectsList.Id.ToString(); LookupField = "Title"; Required = $true } }
    if (-not (Test-Field $list "Role")) { Add-PnPField -List $list -DisplayName "Role" -InternalName "Role" -Type Choice -Choices "PM","SUPERINTENDENT","QTEAM","EXECUTIVE","FINANCE","OTHER" -AddToDefaultView | Out-Null }
    if (-not (Test-Field $list "EffectiveFrom")) { Add-PnPField -List $list -DisplayName "EffectiveFrom" -InternalName "EffectiveFrom" -Type DateTime -AddToDefaultView | Out-Null }
    if (-not (Test-Field $list "EffectiveTo")) { Add-PnPField -List $list -DisplayName "EffectiveTo" -InternalName "EffectiveTo" -Type DateTime -AddToDefaultView | Out-Null }
} catch [System.Management.Automation.RuntimeException] {
    # Test-Field above is the real idempotency guard (Add-PnPField creates Name0
    # rather than throwing). An 'already exists' from PnP is still tolerated.
    if ($_.Exception.Message -notmatch 'already exists') { throw }
