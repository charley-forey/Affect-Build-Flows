...
# One clock read for the run, so every endpoint agrees on whether today is a full-pull day.
run_started = fc.utc_now()
...
for ep in ordered:
  ...
  try:
    ...
    # Weekly full pull (full_pull_weekday in endpoints.yml): an incremental endpoint lists
    # every record on that day, so its deletions can be tombstoned like any full pull.
    scheduled_full = ps.full_pull_due(ep, run_started)
    since = (read_since(ep.bronze_table, ep.name)
             if ep.incremental and not full_parent_discovery and not scheduled_full else None)
    audit["full_parent_discovery"] = full_parent_discovery
    audit["scheduled_full_pull"] = scheduled_full
...
Deletions: a consumed endpoint that pulled completely and returned rows flags bronze keys it
no longer lists with `_source_deleted_at` (`tombstone: true` in the manifest); silver excludes
them. See `_docs/deletion-and-scope-handling.md`.
...
"""
    ...
    cell(
        """
def write(table, rows, tombstone=False):
    df = spark.createDataFrame([[r[c] for c in COLUMNS] for r in rows],
                               ", ".join(f"`{c}` string" for c in COLUMNS))
    df = (df.withColumn("_ingested_at", F.to_timestamp("_ingested_at"))
            .withColumn(fc.DELETED_AT, F.lit(None).cast("timestamp")))
    # merge_delta: creates the table, adds _source_deleted_at to a table landing created, and
    # tombstones keys absent from this complete pull when extract() says the pull is trusted.
    n = fc.merge_delta(spark, df, table, ["_merge_key"],
                       [fc.WHOLE_TABLE] if tombstone else (),
