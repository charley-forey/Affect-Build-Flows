...
def monthly_expected():
    ...
    E.update({
        ...
        "Avg Observation Days To Close": lambda c, s: _avg(R(c, "fct_QualityItem", s,
            lambda r: not r["IsOpen"] and _eq(r["ItemType"], "Observation")), "DaysOpen"),
...
def qc_expected():
    ...
    E.update({
        ...
        "Median Submittal Turnaround Days": lambda c, s: (lambda v: __import__("statistics").median(v) if v else None)(
            [r["TurnaroundDays"] for r in R(c, "fct_QcSubmittal", s, false("IsOpen")) if r["TurnaroundDays"] is not None]),
        "Draft Submittals": count("fct_QcSubmittal", true("IsDraft")),
