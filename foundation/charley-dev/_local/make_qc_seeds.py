...
KPI_OUT = CHARLEY_DEV / "02-transformation" / "sql" / "gold" / "02_seed_kpicatalog.sql"
...
def table_sql(csv_name: str, table: str, columns: dict[str, str],
              key: tuple[str, ...], out_names: tuple[str, ...] | None = None,
              data: list[dict[str, str]] | None = None) -> str:
    """`data` given: rows from somewhere other than a seed CSV, labelled by csv_name."""
    source = csv_name if data is not None else f"seed/{csv_name}"
    data = rows(csv_name, key) if data is None else data
    names = list(columns)
    select = ",\n       ".join(
    ...
    return (
        f"-- {table}: {len(data)} row(s) from {source}\n"
...
def build_kpi_catalog() -> str:
    import kpi_catalog as kc
    columns = {c: "INT" if c == "SortOrder" else "STRING" for c in kc.SEED_COLUMNS}
    return "\n".join([
        "-- gold: seed_KpiCatalog - what each headline KPI on both reports means.",
        "--",
        "-- GENERATED by _local/make_qc_seeds.py from _local/kpi_catalog.py, which also supplies the",
        "-- models' measure descriptions. Do not edit by hand. Bound to the hidden KPI Definitions",
        "-- page in both reports, filtered by ModelName.",
        "",
        table_sql("_local/kpi_catalog.py", "seed_KpiCatalog", columns, ("ModelName", "KpiName"),
                  data=kc.rows()),
    ])
...
def main() -> int:
    ...
    outputs = {OUT: build(), CROSSWALK_OUT: build_crosswalk(), KPI_OUT: build_kpi_catalog()}
    if args.check:
    ...
    for csv_name, table, _, key in (*SEEDS, CROSSWALK):
