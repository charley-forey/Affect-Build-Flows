...
The TMDL is GENERATED by introspecting the offline DuckDB build, which runs the same SQL
Fabric runs. That means the model's columns and types cannot drift from the tables: if a
gold .sql adds a column, the model picks it up on the next run. Hand-maintained TMDL is
how a model ends up referencing a column that no longer exists.
