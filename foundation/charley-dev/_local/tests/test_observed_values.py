...
snake = lambda v: v.strip().lower().replace(" ", "_")  # noqa: E731
...
MAPPINGS = {
    "prime_change_order.status": dict(
        file="gold/21_fct_changeorder.sql", start=r"AS HasOutOfRangeDate,", end=r"AS IsPending",
        norm=snake, default="StatusCategory 'Unknown', IsPending = FALSE (flagged by a WARN DQ rule)",
        default_ok={}, defects={}),
    # Same vocabulary Procore uses on CO packages; not on prime COs yet, but nothing stops it.
    # not_proceeding is one of Procore's documented Pending variants, so it is Pending.
    "potential_change_order.status": dict(
        file="gold/21_fct_changeorder.sql", start=r"AS HasOutOfRangeDate,", end=r"AS IsPending",
        norm=snake, default="StatusCategory 'Unknown'", default_ok={}, defects={}),
    ...
    # One CASE over COALESCE(workflow_status, status_label): both vocabularies are mapped.
    "punch_item.status_label": dict(
        ...
        norm=upper, default="status_code NULL", default_ok={}, defects={}),
    "punch_item.workflow_status": dict(
        file="silver/24_qc_procore_silver.sql", start=r"TABLE cd_silver_qc_punch.*?AS source_status", end=r"AS status_code",
        norm=upper, default="status_code NULL", default_ok={}, defects={}),
    "commitment.status_label": dict(
        file="gold/31_bridge_vendorcostcode.sql", start=r"AS parent_id,\s+COALESCE\(l\.total_amount, l\.amount\)  AS line_amount,\s+CASE",
        end=r"'DRAFT'\)", norm=upper, default="counted as Committed in bridge_VendorCostCode",
        default_ok={v: "a live or finished contract: its value is committed" for v in (
            "APPROVED", "OUT FOR SIGNATURE", "PROCESSING", "CLOSED", "COMPLETE")},
        defects={}),
...
ACCOUNTED = {
    ...
    ("gold/31_bridge_vendorcostcode.sql", "status_label"): ["commitment.status_label"],
    ...
    ("silver/24_qc_procore_silver.sql", "status_label"): ["observation.status_label", "punch_item.status_label",
                                                         "punch_item.workflow_status", "submittal.status_name"],
    ...
    ("gold/30_fct_financialperiod.sql", "StatusCategory"): "category written by 21_fct_changeorder.sql",
    ("snapshot/fct_dailysnapshot.sql", "StatusCategory"): "category written by 21_fct_changeorder.sql",
    ("gold/46_dq_trademappingcandidate.sql", "Kind"): "constant written by seed/qc_trade_synonym_proposals.csv handling in the same file",
    ("gold/46_dq_trademappingcandidate.sql", "MappingStatus"): "constant written in the same file",
    ("gold/46_dq_trademappingcandidate.sql", "MatchedBy"): "constant written in the same file",
    ("gold/46_dq_trademappingcandidate.sql", "SourceType"): "constant written in the same file",
    ("silver/30_manual_silver.sql", "ProjectKey"): "the ALL sentinel (make_sharepoint.py lookup item), not a vocabulary",
    ("silver/30_manual_silver.sql", "project_id"): "the ALL sentinel (make_sharepoint.py lookup item), not a vocabulary",
    ("silver/31_qc_manual_silver.sql", "ProjectKey"): "the ALL sentinel (make_sharepoint.py lookup item), not a vocabulary",
    ("silver/30_manual_silver.sql", "UserPrincipalName"): "UPN format LIKE patterns, not a vocabulary",
}
...
KEY_FIXTURES = {
    ...
    "commitment.status_label": ("sv_commitments", "status_label", upper),
}
