...
| ~~**Outbuild token**~~ | ✅ **Received 2026-08-19** — 3,078 rows across 15 endpoints now land in `cd_bronze_outbuild_*`. Milestones still read the Affect reporting lead's `Silver_Lakehouse` dataflow, so repointing `sv_outbuild_activities` is the remaining step, and it is ours | — |
