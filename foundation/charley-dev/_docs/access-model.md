...
**Verified live 2026-08-24 as the build account.** Every ✅/❌ below is a call that
...
### 1. "It runs on the consultant's laptop" — closeable now, no grant required
...
### 2. "It runs as the consultant's account" — needs grants, and there is a clean version

A scheduled Fabric notebook or pipeline runs as the **item owner**. Today that is
the build account, so the vault read happens as a named consultant. That works and
...
Signed in as the gateway service account and read the gateway. No MFA prompt, no
...
**The Affect reporting lead already holds "Can use" on the exact datasource `CD_Sage_Ingest` needs.** So does
IT. That access was in place before we ever asked for it.

So the ask we have been carrying since 2026-08-02 — *"grant the build account Can use"* — was a
...
owned by the build account, an account with no gateway rights, and then described the resulting
...
The May 20 2026 handoff (from the IT provider, Nerds That Care) is the first full description of the
...
the Affect reporting lead's references to *Affect Build*" and lists `Affect Group` among the alternatives. The
...
The gateway is registered to **the gateway service account** (§4), a service account with
...
nothing in the document grants gateway-admin rights to the Affect reporting lead, to any other Affect staff member, or to any named person.
...
| **B** | Sign in as the gateway service account — whose credentials are now in the vault — and make the grant ourselves | Nothing technical | **Ask before doing.** See below |
...
The gateway service account is a real Entra account, display name *"Fabric Gateway
...
a consultant authenticated as a service account at 2am. **Get one sentence from the Affect reporting lead or
...
**Ask for the identity, not for the consultant.** Every previous ask on this engagement has been
"give the consultant X", which produces a grant that dies with the engagement and has to be re-asked
