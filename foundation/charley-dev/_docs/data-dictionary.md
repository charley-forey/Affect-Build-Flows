...
62 gold tables, 178 measures across 2 semantic models, 213 DQ rules.
...
| Deleted at source | Procore | A Procore record present in bronze but absent from a COMPLETE full pull of its project: deleted (or recycled) in Procore, so silver excluded it from every fact. Listed so a total that dropped overnight can be explained. See _docs/deletion-and-scope-handling.md. |
