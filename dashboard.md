...
| D2 | Procore ETL Validation & Hardening | 1 - Foundation | 🟢 **Complete** - 44 endpoints registered, registry-driven, incremental, tested. **Extraction runs in Fabric as of 2026-08-25**, authenticating from `AffectKeyVault`; the laptop is out of the data path. Five endpoints that had never returned a row now do. Credential rotation still outstanding | 6 | D1 | [D2](deliverables/02-procore-etl-validation.md) |
| D3 | Sage 100 Ingestion Pipeline | 1 - Foundation | 🟢 **Complete and live 2026-08-25** - all 8 tables land (4,027 rows) including the AR/AP line tables, silver layer built, `fct_Invoice` 122 → 148 rows / $25.61M, in the nightly DAG | 2 | - | [D3](deliverables/03-sage100-ingestion.md) |
...
|---|---|---|---|---|
| Procore | API → registry-driven extractor → bronze | 🟢 **44 endpoints registered**, production tenant; `punch_item_types` still 403s and is skipped by design. **`cd_01_extract_procore` runs inside Fabric and is the head of the nightly DAG** - it authenticates from `AffectKeyVault` and merges straight to bronze. Nothing runs on a laptop | **Rotating the exposed Procore credential pair** - the last thing keeping this from clean | D2 |
| Sage 100 Contractor | Dataflow Gen2 over the existing on-prem gateway → bronze | 🟢 **LIVE 2026-08-25.** All 8 tables, **4,027 rows**, including `arivln`/`apivln` — the AR/AP line tables the existing dataflow discards and nobody had ever queried. Runs in the nightly DAG. AP lines carry a GL account on **901 of 901** rows | - | D3 |
...
- [x] ~~**Sage gateway access**~~ - **CLOSED 2026-08-25, and it was never Affect's to grant.** Rebecca and IT already held "Can use"; our dataflow ran as an account that did not. The real blocker was the Lakehouse *destination* being dragged through the on-premises gateway. Sage is live - [`sage-ingestion.md`](foundation/charley-dev/_docs/sage-ingestion.md)
...
| | 🟢 **Done.** `cd_06_land_manual` runs in `CD_Master_Pipeline` as *Land Manual Input* - verified live 2026-08-19. The pipeline is **7 activities**, not 6 - `Extract Procore` joined it 2026-08-25 | — | — |
| ✅ | ~~**`cd_01_extract_procore` added to `CD_Master_Pipeline`**~~ - **done 2026-08-25.** It is the head of the DAG; the nightly run fetches from Procore before it rebuilds | - | - |
...
| ✅ | ~~Retire the local extraction bridge~~ - **done 2026-08-25.** `extract_procore_local.py` stays as a backfill tool, not as the way data arrives | - | - |
